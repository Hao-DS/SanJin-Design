#!/usr/bin/env python3
"""Static, dependency-free checks for SanJin-UI-Agent prototype deliverables.

What this script CAN catch: missing evidence.json, stack mismatch,
cross-stack imports, and a few visual anti-patterns in source text.

What it CANNOT catch (must be verified in the browser):
- empty message content dropped by a Chat component
- a filter that does not actually filter
- unreachable empty / error states
- broken task loops, focus traps, or overlay animation issues

Passing this script is necessary but not sufficient. Standard-mode G2 and
fast-mode F3 still require walking the main task loop in a running page.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate a SanJin-UI-Agent prototype.")
