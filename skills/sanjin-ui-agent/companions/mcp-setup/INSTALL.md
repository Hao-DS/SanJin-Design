...
面向分享「SanJin-UI-Agent」技能后的同事：不装 MCP 仍可做设计方案；**写可运行组件代码时建议至少安装 Semi MCP**。
...
「SanJin-UI-Agent」内已有伴生：`companions/ant-design-x/`。
...
> 用「SanJin-UI-Agent」帮我安装 Semi / Ant Design / TDesign 的 MCP，写入项目 `.cursor/mcp.json`，不要删掉我已有的其它 MCP。
