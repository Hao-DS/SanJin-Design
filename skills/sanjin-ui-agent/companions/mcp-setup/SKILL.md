---
name: mcp-setup
description: 帮助用户在 Cursor 中安装并配置「SanJin-UI-Agent」所需的组件库 MCP（Semi、Ant Design、TDesign）。当用户提到安装 MCP、配置 mcp.json、MCP 不可用、或首次使用SanJin-UI-Agent 写组件代码时使用。
---

# 组件库 MCP 安装助手

本伴生帮助用户把「SanJin-UI-Agent」依赖的组件库 MCP 配进 Cursor，从而能按官方文档取证写代码。
